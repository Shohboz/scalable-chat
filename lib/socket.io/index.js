...
let socketAuth = (socket, next) => {
  ...
  let sid = cookieParser.signedCookie(parseCookie['connect.sid'], config.secret);
...
let removeFromRoom = (socket, room) => {
  ...
  socket.broadcast.to(room).emit('RemoveUser', models.User(socket.user.id, socket.user.displayName, socket.user.provider));
};
...
let socketConnection = (socket) => {
  socket.on('GetMe', () => {
    socket.emit('GetMe', models.User(socket.user.id, socket.user.displayName, socket.user.provider));
  });
  ...
  socket.on('AddChat', (chat) => {
    let newChat = models.Chat(chat.message, chat.room, models.User(socket.user.id, socket.user.provider));
  ...
  socket.on('AddRoom', (r) => {
    ...
    removeAllRooms(socket, () => {
      if (room !== ''){
        ...
        socket.broadcast.to(room).emit('AddUser', models.User(socket.user.id, socket.user.displayName, socket.user.provider));
